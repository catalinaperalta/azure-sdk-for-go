...
type Poller interface {
	...
	// TODO rename params
	FinalResponse(ctx context.Context, pipeline azcore.Pipeline, respType interface{}, field string) (interface{}, error)
	ResumeToken() (string, error)
	PollUntilDone(ctx context.Context, frequency time.Duration, pipeline azcore.Pipeline, respType interface{}, field string) (interface{}, error)
	// NOTE: if we exposed HandleResponse then we can remove the extra fields on FinalResponse and PollUntilDone
}
...
// FinalResponse ...
func (p *pollingTrackerBase) FinalResponse(ctx context.Context, pipeline azcore.Pipeline, respType interface{}, field string) (interface{}, error) {
	if !p.Done() {
	...
	if p.pollerMethodVerb() == http.MethodPut || p.pollerMethodVerb() == http.MethodPatch {
		...
		res, err := p.handleResponse(p.latestResponse(), t, field)
		if err != nil {
			return nil, err
		}
	...
	if p.finalGetURL() == "" {
		...
		if lr := p.latestResponse(); lr != nil && p.hasSucceeded() {
			result, err := p.handleResponse(lr, respType, field)
			if err != nil {
				return nil, err
			}
			return result, nil
	...
	return p.handleResponse(resp, respType, field)
}
...
func (p *pollingTrackerBase) handleResponse(resp *azcore.Response, respType interface{}, field string) (interface{}, error) {
	reflect.ValueOf(respType).FieldByName("RawResponse").Set(reflect.ValueOf(resp.Response))
	...
	return &respType, resp.UnmarshalAsJSON(reflect.ValueOf(respType).FieldByName(field))
}
...
// PollUntilDone ...
func (p *pollingTrackerDelete) PollUntilDone(ctx context.Context, frequency time.Duration, pipeline azcore.Pipeline, respType interface{}, field string) (interface{}, error) {
	...
	return p.FinalResponse(ctx, pipeline, respType, field)
}
...
// NOTE: PollUntilDone can be a private func called from the methods on the poller trackers to remove repeated code
// PollUntilDone ...
func (p *pollingTrackerPatch) PollUntilDone(ctx context.Context, frequency time.Duration, pipeline azcore.Pipeline, respType interface{}, field string) (interface{}, error) {
	// initial check for a retry-after header existing on the initial response
	if retryAfter := azcore.RetryAfter(p.latestResponse().Response); retryAfter > 0 {
		err := delay(ctx, retryAfter)
		if err != nil {
...
// PollUntilDone ...
func (p *pollingTrackerPost) PollUntilDone(ctx context.Context, frequency time.Duration, pipeline azcore.Pipeline, respType interface{}, field string) (interface{}, error) {
	// initial check for a retry-after header existing on the initial response
	if retryAfter := azcore.RetryAfter(p.latestResponse().Response); retryAfter > 0 {
		err := delay(ctx, retryAfter)
		if err != nil {
...
// PollUntilDone ...
func (p *pollingTrackerPut) PollUntilDone(ctx context.Context, frequency time.Duration, pipeline azcore.Pipeline, respType interface{}, field string) (interface{}, error) {
	// initial check for a retry-after header existing on the initial response
	if retryAfter := azcore.RetryAfter(p.latestResponse().Response); retryAfter > 0 {
		err := delay(ctx, retryAfter)
		if err != nil {
...
// CreatePollingTracker creates a polling tracker based on the verb of the original request
...
// ResumePollingTracker creates a polling tracker from a resume token
